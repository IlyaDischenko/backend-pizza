...
def check_percent(promo, number):
    sel = select([discount_percent.c.percent, discount_percent.c.count, discount_percent.c.is_view,
                  discount_percent.c.type, discount_percent.c.number]).where(discount_percent.c.promocode == promo)
    res = conn.execute(sel).fetchall()

    if len(res) < 1:
        return {"status": 400}
    elif len(res) != 0:
        if res[0][2] and res[0][1] != 0:
            if res[0][3]:
                return {"percent": res[0][0], "status": 200}
            elif not res[0][3] and res[0][4] != number:
                return {"status": 401}
            elif not res[0][3] and res[0][4] == number:
                return {"percent": res[0][0], "status": 200}
